...
class ExamScheduler(QWidget):
    ...
    def _build_ui(self):
        ...
        self.date_to   = QDateEdit(QDate.currentDate().addDays(14)); self.date_to.setCalendarPopup(True)
        f2.addRow("Başlangıç:", self.date_from)
        f2.addRow("Bitiş:", self.date_to)

        days_row = QHBoxLayout()
        # hafta içi default açık, hafta sonu hariç tutulabilir
        self.chk_mon = QCheckBox("Pzt"); self.chk_mon.setChecked(True)
        self.chk_tue = QCheckBox("Sal"); self.chk_tue.setChecked(True)
        self.chk_wed = QCheckBox("Çar"); self.chk_wed.setChecked(True)
        self.chk_thu = QCheckBox("Per"); self.chk_thu.setChecked(True)
        self.chk_fri = QCheckBox("Cum"); self.chk_fri.setChecked(True)
        self.chk_sat = QCheckBox("Cmt"); self.chk_sat.setChecked(False)
        self.chk_sun = QCheckBox("Paz"); self.chk_sun.setChecked(False)
        ...
        self.cmb_type = QComboBox(); self.cmb_type.addItems(["Vize", "Final", "Bütünleme"])
        self.spin_duration = QSpinBox(); self.spin_duration.setRange(30, 240); self.spin_duration.setValue(75)
        self.spin_break = QSpinBox(); self.spin_break.setRange(0, 120); self.spin_break.setValue(15)
    ...
    def _on_generate_clicked(self):
        ...
        try:
            ...
            for ders in cons["dersler"]:
                ...
                yerlesmis = False
                for day in days:
                    # Aynı sınıf o gün sınav yapmış mı?
                    ...
                    for slot in DEFAULT_SLOTS:
                        ...
                        for r in rooms:
                            if r["kapasite"] >= kapasite:
                                ...
                                break
                        if not secilen:
                            raise ValueError(f"{ders['ders_adi']} için kapasite yetersiz!")
                ...
                if not yerlesmis:
                    raise ValueError(f"{ders['ders_adi']} yerleştirilemedi, uygun gün/slot bulunamadı.")
...
VALUES (%s, %s, %s, %s, %s, %s, %s, 'Planlandı')
            ...
            rows = Database.execute_query("""
                SELECT d.ders_kodu, d.ders_adi, s.sinav_tarihi, s.sinav_saati, d.sinif, dl.derslik_adi
                FROM Sinavlar s
            ...
            for i, r in enumerate(rows):
                ...
                self.preview.setItem(i, 2, QTableWidgetItem(r["sinav_tarihi"].strftime("%d.%m.%Y")))
                self.preview.setItem(i, 3, QTableWidgetItem(r["sinav_saati"].strftime("%H:%M")))
                self.preview.setItem(i, 4, QTableWidgetItem(str(r["sinif"])))
                self.preview.setItem(i, 5, QTableWidgetItem(r["derslik_adi"] or "-"))
    ...
    def export_to_excel(self):
        ...
        try:
            ...
            conn = psycopg2.connect(
                host="localhost",
                database="student_schedule_system",
                user="proje_kullanicisi",
                password="yazlab"
            )
            cur = conn.cursor()
            ...
            ws["A1"].fill = PatternFill("solid", fgColor="F4B084")
            ...
            current_row = 3
            renk_index = 0  # Gün bazlı renk geçişi için sayaç
            renkler = [
                "FFFFFF",  # Beyaz
                "FFF2CC"  # Açık sarı / turuncu ton (istenirse değiştirilebilir)
            ]
            ...
            for tarih, grup in df.groupby("Tarih"):
                ...
                date_cell.fill = gun_rengi  # Aynı arka plan

                renk_index += 1  # Bir sonraki gün için renk değiştir
    ...
    def assign_exam_rooms(self):
        """SinavDerslikleri tablosunu otomatik doldurur - aynı gün ardışık sınav çakışması olmadan"""
        try:
            import psycopg2
            ...
            # 🔹 Bölüme ait sınavları tarih ve saate göre sırala
            cur.execute("""
                SELECT sinav_id, sinav_tarihi, sinav_saati
                FROM Sinavlar
                WHERE bolum_id = %s AND durum = 'Planlandı'
                ORDER BY sinav_tarihi, sinav_saati
            """, (self.user["bolum_id"],))
            ...
            # 🔹 Derslik program geçmişi (tarih bazlı saat listesi)
            derslik_programi = {d[0]: {} for d in derslikler}
            # örnek: { 3: { date(2025,10,21): [time(10,0), time(12,30)] } }

            atanan = 0
            for sinav_id, tarih, saat in sinavlar:
                if isinstance(saat, str):
                    saat = datetime.strptime(saat, "%H:%M").time()
                ...
                for derslik_id, derslik_adi, kapasite in derslikler:
                    ...
                    gunluk_program = derslik_programi[derslik_id].get(tarih, [])
                    # Aynı saatte veya hemen önce/sonra çakışma var mı kontrol
                    conflict = False
                    for s in gunluk_program:
                        fark = abs(
                            (datetime.combine(tarih, saat) - datetime.combine(tarih, s)).total_seconds()
                        )
                        if fark < 90 * 60:  # 1.5 saat içinde sınav varsa çakışma kabul
                            conflict = True
                            break
                    if conflict:
                        continue

                    uygun_derslik = derslik_id
                    derslik_programi[derslik_id].setdefault(tarih, []).append(saat)
                    break
                ...
                if uygun_derslik:
                    ...
                    """, (sinav_id, uygun_derslik))
                    atanan += 1
                else:
                    print(f"❌ {tarih} {saat} için uygun derslik bulunamadı.")

            conn.commit()
            cur.close()
            conn.close()
            print(f"✅ {atanan} sınav için uygun derslik atandı.")
